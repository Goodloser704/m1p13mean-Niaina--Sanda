...
async function creerEtage() {
  ...
  // Utiliser un numéro aléatoire entre 10 et 49
  const numeroUnique = Math.floor(Math.random() * 40) + 10;
  const etageData = {
    numero: numeroUnique,
  ...
  try {
    ...
    // Si le numéro existe, réessayer avec un autre
    if (error.response?.data?.message?.includes('existe déjà')) {
      console.log('⚠️ Numéro existe, réessai...');
      const autreNumero = Math.floor(Math.random() * 40) + 10;
