/**
 * Test complet de la logique de suppression - LOCAL
 * 
...
async function supprimerBoutique(boutiqueId) {
  const response = await axios.delete(`${BASE_URL}/boutique/${boutiqueId}`, {
    headers: { Authorization: `Bearer ${commercantToken}` }
  });
  return response.data;
}
  ...
  try {
    ...
    try {
      ...
      if (error.response?.data?.code === 'ROUTE_NOT_FOUND') {
        console.log('⚠️ TEST 4: Route POST notifications non disponible');
